...
## All latent model (Failed)
```{r}
observed_variables <- c('pa', 'ran', 'pm', 'nviq', 'vocab', 'ef', 'number_knowledge', 'calculation', 'word_problem')
data <- prep_input(df, observed_variables)
lavaan_spec <- 'pho =~ pa + ran + pm
                cog =~ nviq + ef
                math =~ number_knowledge + calculation + word_problem
                cog ~~ pho
                cog ~~ vocab
                pho ~~ vocab
                math ~ cog + pho + vocab'
...
stage1<-tssem1(data$cor, data$n, method="REM", RE.type="Diag") 


summary(stage1)
coef(stage1, select="random")


stage2 <- tssem2(stage1, RAM=RAM_spec) 
summary(stage2)
plot(stage2)
